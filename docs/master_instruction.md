...
## A02_fina Research 責務 (session-a02-research-01)

A02_fina は記事生成に先立ち、以下 3 レイヤーに責務を分離する。Research レイヤーは最上位入力を「悩み」とし、keyword 起点は禁止する。

1. **Research**: 対象 vertical の主要な悩みを起点に、Know / Do / Buy / Go の 4 意図に分解する。keyword 起点は不採用とする。
2. **Design**: Research 出力を受け、クラスタ設計 / カニバリチェック / 公開優先度を定義する。
3. **Generate**: Design 出力を受け、テンプレート選択・下書き生成・検証へ進む。

session-a02-research-01 は Research のみを対象とし、Design / Generate / 本文生成を含まない。
