> **v1 凍結 (2026-Q2)**
>
> 本ドキュメントは旧・軽量2AIオーケストレーター方式 v1 の正本です。
> v2 体制への移行に伴い **参照のみ・改訂禁止** とします。
> 打ち切り・凍結線の一覧は `docs/archive/v1-2026-Q2-README.md` を正とします。
...
Cursor は本プロジェクトにおける実装・実行担当であり、仕様確定・最終承認は担当しない。
Cursor 実行時は、以下を必須ルールとする。

- 指示には必ず停止点を含める（例: 「まず差分確認まで」「ここで停止して報告」）
- 起票コミットと実装コミットは必要に応じて分離し、混在を避ける
- 依頼されていない自動生成（現状サマリー、バックログ一覧等）を行わない
- scope 外ファイルを変更しない。forbidden_changes は実行前に明示・再確認する
- 指示にない Git 操作（commit / push / rebase / merge / cherry-pick / stash）は行わない
- 実行前に最低限 `branch` / `git status` / `git config` / 対象 commit / clean worktree を確認する

本ガードは、main 汚染防止・再現性確保・統合コミット化防止・誤前提混入防止を目的とする。
